...
import { JOUE_AVEC_DATA } from '../data/joueAvecData';
import { ArrowLeft, Users, CheckCircle2, XCircle } from 'lucide-react';

export default function JoueAvec({ onBack }) {
  ...
  const drawPuzzle = () => {
    let available = JOUE_AVEC_DATA.filter((_, i) => !playedIds.includes(i));
    if (available.length === 0) {
      available = JOUE_AVEC_DATA;
      setPlayedIds([]);
      setScore(0);
      setTotalPlayed(0);
    }
    ...
    const realIndex = JOUE_AVEC_DATA.indexOf(selected);
  ...
  const handleSelect = (index) => {
    ...
    if (index === currentPuzzle.answerIndex) {
      setScore(s => s + 1);
    }
  };
  ...
  return (
    <div className="glass-panel animate-fade-in" style={{ margin: 'auto', width: '100%', maxWidth: '700px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      ...
      <h2 className="title" style={{ fontSize: '2rem', marginBottom: '0.5rem' }}>{t('joueavec.title')}</h2>
      <p style={{ color: 'var(--text-secondary)', marginBottom: '2rem', textAlign: 'center' }}>
        {t('joueavec.subtitle')}
      </p>

      {/* Les 4 coéquipiers */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', width: '100%', marginBottom: '3rem' }}>
        {currentPuzzle.teammates.map((teammate, i) => (
      ...
      <h3 style={{ marginBottom: '1rem', color: 'var(--text-secondary)' }}>{t('joueavec.choose_answer')}</h3>
          ...
          return (
            <button
              ...
              style={{
                ...
                fontSize: '1.2rem',
                fontWeight: 'bold',
                color: isRevealed && index !== currentPuzzle.answerIndex && index !== selectedIndex ? 'var(--text-secondary)' : 'white',
      ...
      {isRevealed && (
        <div className="animate-pop" style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem' }}>
          <div style={{ 
            background: selectedIndex === currentPuzzle.answerIndex ? 'rgba(16, 185, 129, 0.1)' : 'rgba(239, 68, 68, 0.1)',
            border: `2px solid ${selectedIndex === currentPuzzle.answerIndex ? 'var(--success)' : 'var(--danger)'}`,
            padding: '1.5rem',
            borderRadius: '16px',
            textAlign: 'center',
            width: '100%'
            ...
            <h3 style={{ 
              color: selectedIndex === currentPuzzle.answerIndex ? 'var(--success)' : 'var(--danger)', 
              margin: '0 0 1rem 0',
              fontSize: '1.5rem',
              fontWeight: 'bold'
            }}>
              {selectedIndex === currentPuzzle.answerIndex ? t('joueavec.correct') : t('joueavec.wrong')}
            </h3>
          ...
          <button className="btn btn-primary" onClick={drawPuzzle} style={{ padding: '1rem 2rem', fontSize: '1.1rem', width: '100%' }}>
            {t('joueavec.next_player')} <ArrowLeft size={18} style={{ transform: 'rotate(180deg)' }} />
          </button>
        </div>
      )}
